...
#**Load dataset**
pri_df<- fread("pri_Cd_data.csv", header=TRUE, sep=";", skip = 2, 
                    fill=TRUE) 
pri_df[, `:=`(
  logclay = log10(Clay),
  logSOC = log10(OC/10),
  logsolutionCd = log10(SoilCdavi*10),
  logsoilCd = log10(SoilCdtot)
)]                    
...
soil_solution[, pred := ifelse(land_use == "Upland", solu_coeffs["logsoilCd"]*logsoilCd + solu_coeffs["logSOC"]*logSOC + solu_coeffs["logclay"]*logclay + solu_coeffs["pH"]*pH + solu_coeffs["land_useUpland"], 
                                                     solu_coeffs["logsoilCd"]*logsoilCd + solu_coeffs["logSOC"]*logSOC + solu_coeffs["logclay"]*logclay + solu_coeffs["pH"]*pH + solu_coeffs["land_usePaddy"])]
...
# Derive predictions based on the multiple linear regression
crop_data[, pred := ifelse(crop == "straw", crop_coeffs["logsoilCd"]*logsoilCd + crop_coeffs["logSOC"]*logSOC + crop_coeffs["logclay"]*logclay + crop_coeffs["pH"]*pH + crop_coeffs["cropstraw"], 
                                      crop_coeffs["logsoilCd"]*logsoilCd + crop_coeffs["logSOC"]*logSOC + crop_coeffs["logclay"]*logclay + crop_coeffs["pH"]*pH + crop_coeffs["cropgrain"])] 
                    

#**Make plot**
# Combine metrics and equation for annotation
crop_annotation_text <- paste0(
  crop_regression_eq, "\n",
...
library(ggsci)
p1 <- ggplot(crop_data)+
  geom_point(aes(x = logcropCd, y = pred, 
                 color = crop, shape = source, alpha = outlier),
             size = 3) +  # Points
...
# Combine metrics and equation for annotation
solu_annotation_text <- paste0(
  solu_regression_eq, "\n",
  "R² = ", round(summary(ML_solupred)$r.squared, 2), "\n",
...
p2 <- ggplot(soil_solution)+
  geom_point(aes(x = logsolutionCd, y = pred, 
                 color = land_use, shape = source, alpha = outlier),
             size = 3) +  # Points
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed", linewidth = 1) +  # Line of perfect fit
  labs(
...
# combined_plot
ggsave(combined_plot, filename = "regression_model.tiff",
  width = 11,
  height = 5,
  units = "in",
  dpi = 300
